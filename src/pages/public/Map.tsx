    ...
    <section id="map" className="relative py-24 px-6 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        <div className="text-center mb-10">
          <h2 className="text-4xl md:text-5xl font-bold text-amber-300 mb-3">Mappa del Mondo</h2>
        ...
        >
          <img src="/Mappa-Sherdan.png" alt="Mappa di Sherdan" className="w-full h-full object-cover" draggable={false} />
          ...
          <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
            ...
            {MAP_LOCATIONS.map((loc) => {
              ...
              return (
                <circle
                  key={loc.id} cx={loc.x} cy={loc.y} r={r}
                  fill={isHovered ? (isRegion ? 'rgba(251,191,36,0.18)' : 'rgba(147,210,255,0.15)') : 'transparent'}
                  stroke="none"
                  style={{ cursor: 'pointer', filter: isHovered ? (isRegion ? 'url(#glow-region)' : 'url(#glow-city)') : 'none', transition: 'fill 0.25s ease' }}
          ...
          {hovered && (() => {
            ...
            return (
              <div
                ...
                style={{
                  left: `${loc.x}%`, top: `${loc.y - (loc.r ?? 3) - 2}%`,
                  transform: 'translateX(-50%) translateY(-100%)',
                  background: 'rgba(15,23,42,0.85)',
                  color: loc.type === 'region' ? '#fde68a' : '#bae6fd',
                  border: `1px solid ${loc.type === 'region' ? 'rgba(251,191,36,0.4)' : 'rgba(147,210,255,0.3)'}`,
        ...
        {selected && (
          <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center px-4" onClick={() => setSelected(null)}>
            <div className="max-w-lg w-full bg-slate-900 border border-amber-700/30 rounded-2xl p-8 shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center gap-3 mb-2">
                <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${selected.type === 'region' ? 'bg-amber-400' : 'bg-sky-300'}`} />
                <span className="text-xs uppercase tracking-widest text-slate-400">{selected.type === 'region' ? 'Regione' : 'Città'}</span>
              </div>
              ...
              <div className="flex gap-3 mt-8">
                {selected.type === 'region' && (
                  <button onClick={handleShowMore} className="px-5 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-xl font-semibold transition">Mostra altro →</button>
                )}
                <button onClick={() => setSelected(null)} className="px-5 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-xl transition">Chiudi</button>
              </div>
            </div>
          </div>
        )}
