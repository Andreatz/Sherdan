...
import { BookOpen, Save, Plus, Trash2, ChevronDown, ChevronUp, Lock } from 'lucide-react';
  ...
  return (
    <div className={`bg-slate-800 border rounded-2xl overflow-hidden transition-all ${
      ...
      <div className="flex items-center gap-3 px-4 py-3 cursor-pointer select-none"
        ...
        <span className="flex-1 font-semibold text-slate-100 truncate">{title || 'Nota senza titolo'}</span>
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3 border-t border-slate-700/40">
          ...
          <textarea
            ...
            rows={6}
            placeholder="Scrivi qui i tuoi appunti segreti..."
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={() => onDelete(note.id)}
                className="p-2 text-slate-600 hover:text-red-400 transition" title="Elimina">
                <Trash2 size={15} />
              </button>
      ...
      <div className="min-h-screen bg-slate-900 pt-24 pb-16 px-4">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-4xl mb-1">Note di Sessione</h1>
              <p className="text-slate-500 text-sm flex items-center gap-1">
                <Lock size={12} /> Visibili solo a te
              </p>
            </div>
            <button onClick={handleCreate} disabled={creating}
              ...
              <Plus size={16} /> {creating ? '...' : 'Nuova nota'}
            </button>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-slate-500 mb-2">Nessuna nota ancora.</p>
              <p className="text-slate-600 text-sm">Crea la tua prima nota per tenere traccia di segreti, obiettivi e NPC importanti.</p>
            </div>
          ) : (
