  ...
  useEffect(() => {
    const fetchMissions = async () => {
      const { data } = await supabase
        .from('missions')
        .select('id, title, description, status, characters(name)')
      ...
      if (data) {
        setMissions(
          data.map((m: any) => ({
            ...
            status: m.status,
            npc_name: m.characters?.name ?? null,
          }))
        );
      }
      setLoading(false);
                ...
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-sm font-semibold transition capitalize ${
                    filter === s
                      ...
                      : 'bg-slate-800/70 text-amber-200 hover:bg-slate-700'
                  }`}
                >
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {filtered.map((mission) => (
                  <div
                    key={mission.id}
                    className={`bg-slate-900/80 backdrop-blur-sm border border-amber-700/20 rounded-xl p-6 ${
