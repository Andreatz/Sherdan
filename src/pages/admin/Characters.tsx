...
export const CharactersPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...formData,
        owner_email: formData.is_player_character
          ? formData.owner_email.trim().toLowerCase() || null
          : null,
          ...
          <div className="bg-slate-800 border border-amber-700/20 rounded-xl p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex items-center gap-6 p-4 bg-slate-700/50 rounded-xl border border-amber-700/20">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={() => setFormData({ ...formData, is_player_character: false })}
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    name="char_type"
                    checked={formData.is_player_character}
                    onChange={() => setFormData({ ...formData, is_player_character: true })}
                    className="accent-amber-500"
                  />
                  <span className="text-slate-200 flex items-center gap-1">
                    <User size={14} /> Personaggio Giocante (PG)
                  </span>
                </label>
              </div>
              ...
              <div>
                ...
                <textarea
                  placeholder="Versione pubblica della storia (visibile a tutti)"
                ...
                <div className="space-y-4 border border-amber-600/30 bg-slate-700/30 rounded-xl p-5">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        placeholder="es. Sigillo di Mitra – L'Ombra"
                        value={formData.sigillo}
                  ...
                  <div>
                    ...
                    <textarea
                      placeholder="Backstory privata, segreti sul sigillo, trame personali..."
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                ...
                <button
                  type="button"
                  onClick={resetForm}
                  className="bg-slate-600 hover:bg-slate-500 text-white font-semibold py-2 px-6 rounded transition"
                >
                ...
                <div className="flex gap-4 flex-1 min-w-0">
                  {character.portrait_url ? (
                    <img
                      ...
                      alt={character.name}
                      className="w-16 h-16 rounded-xl object-cover flex-shrink-0 border border-amber-700/30"
                    />
                  ) : (
                    <div className="w-16 h-16 rounded-xl bg-slate-700 flex items-center justify-center flex-shrink-0 text-slate-500">
                      <User size={24} />
                    </div>
                  )}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {character.is_player_character ? (
                        <span className="inline-flex items-center gap-1 text-xs bg-amber-900/40 text-amber-400 border border-amber-700/40 rounded px-2 py-0.5">
                          <User size={10} /> PG
                        </span>
                      ) : (
                    ...
                    {character.is_player_character && character.owner_email && (
                      <p className="text-slate-500 text-xs mt-1">📧 {character.owner_email}</p>
                    )}
                    {character.is_player_character && character.owner_user_id && (
                      <p className="text-green-500 text-xs mt-0.5">✅ Account collegato</p>
                    )}
                    {character.is_player_character && !character.owner_user_id && character.owner_email && (
                      <p className="text-yellow-500 text-xs mt-0.5">⏳ In attesa del primo login</p>
                    )}
                    {character.sigillo && (
