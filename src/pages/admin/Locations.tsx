...
export const LocationsPage: React.FC = () => {
  ...
  const resetForm = () => {
    setFormData({ name: '', description: '', location_type: 'port', x_coordinate: 0, y_coordinate: 0, control_status: 'neutral', history: '' });
    setEditingId(null);
    setShowForm(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        const { error } = await supabase.from('world_locations').update(formData).eq('id', editingId);
        if (error) throw error;
      } else {
        const { error } = await supabase.from('world_locations').insert([formData]);
        if (error) throw error;
      }
      resetForm();
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const { error } = await supabase.from('world_locations').delete().eq('id', id);
      if (error) throw error;
      await fetchLocations();
    } catch (error) {
