...
import { Users, BookOpen, Map, Image, Settings, Skull } from 'lucide-react';
...
export const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const [charRes, sessRes, locRes, galRes, creRes] = await Promise.all([
  ...
  const quickLinks = [
    ...
    { title: 'Bestiario',                        text: 'Gestisci le creature incontrate dalla compagnia. Rivelale ai giocatori dopo ogni incontro.', path: '/admin/bestiary', icon: Skull },
  ...
  return (
    <AdminLayout>
      <div className="space-y-8">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold text-amber-300 mb-2">{it.dashboard.title}</h1>
          <p className="text-slate-300">{it.dashboard.welcomeText}</p>
        </div>

        {isLoading ? (
          <div className="text-center text-amber-300 py-10">{it.dashboard.loading}</div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4">
            {statCards.map((card) => {
              const Icon = card.icon;
              return (
                <div key={card.label} className="bg-slate-800 border border-amber-700/20 rounded-xl p-5 shadow-lg">
        ...
        <div className="bg-slate-800 border border-amber-700/20 rounded-xl p-6">
          <h2 className="text-2xl font-bold text-amber-300 mb-5">{it.dashboard.quickLinks}</h2>
            ...
            {quickLinks.map((link) => {
              ...
              return (
                <a key={link.path} href={link.path}
                  ...
                  <p className="text-slate-300 text-sm leading-6">{link.text}</p>
                </a>
              );
            })}
          </div>
        ...
        <div className="bg-slate-800 border border-amber-700/20 rounded-xl p-6">
          <h2 className="text-2xl font-bold text-amber-300 mb-3">{it.dashboard.welcomeTitle}</h2>
          <p className="text-slate-300 leading-7">{it.dashboard.welcomeText}</p>
        </div>
      </div>
    </AdminLayout>
  );
