...
import { Menu, X, LogOut, Settings, Scroll, Sword, MapPin, Skull } from 'lucide-react';
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-slate-950/90 backdrop-blur border-b border-amber-700/20">
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          <div className="flex items-center justify-between h-16">
            ...
            {/* Logo */}
            <button onClick={() => navigate('/')} className="flex items-center hover:opacity-80 transition" aria-label="Torna alla home">
            ...
            <div className="hidden md:flex items-center gap-6 text-slate-200">
              ...
              <button onClick={() => navigate('/luoghi')}
                className="flex items-center gap-1 hover:text-amber-300 transition">
                <MapPin size={14} /> Luoghi
              </button>
              <button onClick={() => navigate('/bestiario')}
                className="flex items-center gap-1 hover:text-amber-300 transition">
                <Skull size={14} /> Bestiario
              </button>
              ...
              {user && !isAdmin && (
                <>
                  <button onClick={() => navigate('/personaggio')}
                    ...
                    <Scroll size={15} /> {it.nav.myCharacter}
                  </button>
                  <button onClick={() => navigate('/missioni')}
                    className="flex items-center gap-1.5 text-amber-400 hover:text-amber-300 font-semibold transition">
                    <Sword size={15} /> Missioni
                  </button>
                </>
              )}
            </div>
          ...
          {isOpen && (
            <div className="md:hidden pb-4 space-y-2 border-t border-amber-700/20 pt-4">
              ...
              {user && !isAdmin && (
                <>
                  <button onClick={() => { navigate('/personaggio'); closeMobileMenu(); }}
                  ...
                  <button onClick={() => { navigate('/missioni'); closeMobileMenu(); }}
                    className="w-full text-left flex items-center gap-2 px-4 py-2 text-amber-400 font-semibold hover:bg-slate-800 rounded transition">
                    <Sword size={15} /> Missioni
                  </button>
                </>
              ...
              {!user && (
                <button onClick={() => { navigate('/auth/login'); closeMobileMenu(); }}
                  className="w-full text-left px-4 py-2 text-amber-100 hover:bg-slate-800 rounded transition">
                  {it.nav.login}
                </button>
              )}
              {isAdmin && (
                <button onClick={() => { navigate('/admin'); closeMobileMenu(); }}
                  className="w-full text-left px-4 py-2 text-amber-100 hover:bg-slate-800 rounded transition">
                  {it.nav.adminDashboard}
                </button>
              )}
              {user && (
                <button onClick={async () => { await handleSignOut(); closeMobileMenu(); }}
